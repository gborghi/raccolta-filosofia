# -*- coding: utf-8 -*-
"""Traduce tutti gli atomi pendenti di Ortega y Gasset con DeepSeek API.

    DEEPSEEK_API_KEY=sk-... python scripts/translate/run_ortega_deepseek.py
    DEEPSEEK_API_KEY=sk-... DEEPSEEK_WORKERS=16 python scripts/translate/run_ortega_deepseek.py

Riprendibile: salva progress.json e una cache dei blocchi tradotti.
Un atomo che ha gia' il sibling non viene mai riconsiderato.
"""
import hashlib
import json
import os
...
PENDING_PATH = os.path.join(ROOT, "data", "ortega_pending.json")
CACHE_PATH = os.path.join(ROOT, "data", "ortega_translate_cache.jsonl")
PROGRESS_PATH = os.path.join(LOG_DIR, "ortega_progress.json")
REJECTED_PATH = os.path.join(LOG_DIR, "ortega_rejected.jsonl")

DEEPSEEK_HOST = os.environ.get("DEEPSEEK_HOST", "https://api.deepseek.com")
DEEPSEEK_MODEL = os.environ.get("DEEPSEEK_MODEL", "deepseek-v4-flash")
API_KEY = os.environ.get("DEEPSEEK_API_KEY", "")
WORKERS = int(os.environ.get("DEEPSEEK_WORKERS", "12"))

# ---------------------------------------------------------------------------
# wikilink masking -- identico a hy_tower.py
# ---------------------------------------------------------------------------
...
# API call
# ---------------------------------------------------------------------------
def call_deepseek(messages, max_tokens, temperature=0):
    body = json.dumps({
    ...
    for attempt in range(5):
        try:
            req = urllib.request.Request(
                DEEPSEEK_HOST + "/v1/chat/completions",
                data=body,
                headers={
    ...
    raise RuntimeError("DeepSeek call failed: %s" % last_err)
...
def translate_block(src_block, tgt_lang, src_lang, cache, tries=3):
    """Traduce un blocco di prosa. Ritorna (testo, mancanti)."""
    ...
    if not want:
        ...
        if cached:
            return cached, []
        # Blocchi cortissimi (titoli di una parola, "(intero)", numeri romani):
        # la traduzione e' prevedibile, ma il modello potrebbe rifiutare input
        # troppo corto. Passiamo comunque dal modello; se la risposta e' vuota o
        # nonsense, teniamo il testo originale (meglio che bloccare l'atomo).
        if len(src_block) < 60:
            msgs = [
            ...
            try:
                ...
                if len(out) >= 2 and not _is_fabricated(src_block, out):
                    cache.put(src_block, tgt_lang, out)
                    return out, []
            except Exception:
                pass
            # Fallback: per titoli cortissimi teniamo l'originale
            cache.put(src_block, tgt_lang, src_block)
            return src_block, []
        ...
        for _ in range(tries):
            ...
            if not _is_fabricated(src_block, out):
                cache.put(src_block, tgt_lang, out)
                return out, []
        raise RuntimeError("fabricated output after %d tries" % tries)
    ...
    if best is None:
        raise RuntimeError("fabricated output on all %d tries" % tries)
...
def _is_fabricated(src_block, tr_block):
    # Blocchi molto corti (titoli, H1, etc.) possono variare moltissimo: le
    # regole di capitalizzazione cambiano fra lingue, e il traduttore a volte
    # esplicita l'implicito — "PRÓLOGO PARA FRANCESES" puo' diventare
    # "PROLOGO PER I FRANCESI" o "PREFAZIONE PER I FRANCESI", entrambi validi.
...
def translate_atom(src_path, tgt_lang, cache):
    """Corpo tradotto di un atomo intero. Ritorna (corpo, mancanti)."""
...
# ---------------------------------------------------------------------------
# validazione allineata a verify.py
# ---------------------------------------------------------------------------
...
def process_one(job, cache):
    """Traduce un atomo in una lingua. Ritorna (ok, reject, fail)."""
    src_path = job["src"]
    lang = job["lang"]
    fname = os.path.basename(src_path)
    try:
        # skip se gia' esiste (ripresa)
        ...
        if problems:
            record_reject(src_path, lang, problems)
        ...
        record_reject(src_path, lang, ["exception: %s" % e])
...
def main():
    ...
    # Filtra via quelli gia' fatti (ripresa dopo interruzione)
    remaining = []
    for j in pending:
        if not os.path.exists(sibling_path(j["src"], j["lang"])):
            remaining.append(j)
    log("### Ortega y Gasset: %d traduzioni pendenti su %d totali (%.1f%% gia' fatti)"
    ...
    elapsed = time.time() - t0
    log("\n### Ortega y Gasset completato in %.0fs (%.1f min)" % (elapsed, elapsed / 60))
