#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Traduce atomi pendenti di un filosofo con DeepSeek API (v4-flash).

    DEEPSEEK_API_KEY=sk-... python scripts/translate/run_author_ds.py Augustine
    DEEPSEEK_API_KEY=sk-... DEEPSEEK_WORKERS=16 python scripts/translate/run_author_ds.py Augustine
        ...
        if len(src_block) < 60:
            ...
            try:
                ...
                if len(out) >= 2 and not _is_fabricated(src_block, out):
                    # Rifiuto/corruzione check per blocchi corti
                    refusal_markers = [
                        "I am sorry", "I cannot", "I can't", "unable to",
                        "Mi dispiace", "Non posso", "I apologize",
                    ]
                    is_refusal = any(m.lower() in out.lower() for m in refusal_markers)
                    if is_refusal:
...
def main():
    ...
    if len(sys.argv) < 2:
        print("USO: python scripts/translate/run_author_ds.py <NomeFilosofo>")
        print("Es:   python scripts/translate/run_author_ds.py Augustine")
        return 1

    author = sys.argv[1]
    cache_path = os.path.join(ROOT, "data", "ds_translate_cache_%s.jsonl" % author.lower())
    rejected_path = os.path.join(LOG_DIR, "ds_rejected_%s.jsonl" % author.lower())
